...
def scrape_amazon(search_query):
    ...
    for listing in listings[:2]:
        ...
        rating_element = listing.find("div", {"class": "a-row a-size-small"}).find('span', class_ = "a-icon-alt")
        if rating_element:
...
def scrape_snapdeal(search_query):
    ...
    for listing in listings[:2]:      
        ...
        rating_element = listing.find("div", {"class" : "filled-stars"}).get('style')
        if rating_element:
            ratingLst = rating_element.split(":");
